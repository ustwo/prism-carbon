...
- **Runtime** — an HTTP proxy intercepts outbound LLM API calls; proxy env vars are injected into every new VS Code terminal so scripts run without any manual configuration.
...
This compiles the source and produces `PRISM.vsix` inside `vsCodeExt/distExtension/`.
...
```bash
code --install-extension "distExtension/PRISM.vsix"
```
...
├── ProjectNotes/                  # meeting notes, research, and documents
...
- **Handover Document** — [`ProjectNotes/Documents/Handover Document.pdf`](ProjectNotes/Documents/Handover%20Document.pdf): setup instructions, features, maintenance guidance, future development, and known issues.
- **Methodology Report** — [`ProjectNotes/Documents/Emission Calculation Methodology.pdf`](ProjectNotes/Documents/Emission%20Calculation%20Methodology.pdf): detailed explanation of the carbon estimation approach.
