...
const matchers_1 = require("mockttp/dist/rules/matchers");
...
async function startServer(port, storagePath) {
    try {
        ...
        await server.forAnyRequest().thenPassThrough({
            ...
            beforeRequest: async (req) => {
                ...
                if (body) {
                    sendLog(`Body: ${body.substring(0, 200)}...`);
                }
                ;
                sendLog('------------------------------------------------');
            },
            beforeResponse: async (res) => {
                ...
                try {
                    ...
                    const preview = matchers_1.RawBodyIncludesMatcher.length > 2000
        ...
        if (process.send) {
            process.send({ type: 'started', certPath });
        }
        ;
    }
    catch (error) {
...
// token detection
function detectTokens(headers, body) {
...
function scanJsonForTokens(obj) {
    ...
    Object.keys(obj).forEach(key => {
        ...
        if (typeof value === 'object') {
            scanJsonForTokens(value);
        }
        ;
    });
}
function getJsonTokenCount(body) {
...
function calculateEmission(model, token) {
